 ...
 * @author Marti
 */
public class Queen implements Figure, Cloneable {
     ...
     */
    public Queen(int x, int y, boolean white, int width, int height) {
        this.X = x;
        this.Y = y;
        this.White = white;
        if (white) {
    ...
    public ArrayList<Point> getPossibleTurns(ArrayList<Figure> figures, Figure ignorated) {
        ...
        ArrayList<Point> pt1 = new Rook(this.X, this.Y, this.isWhite(), 0, 0).getPossibleTurns(figures, ignorated);
        ArrayList<Point> pt2 = new Bishop(this.X, this.Y, this.isWhite(), 0, 0).getPossibleTurns(figures, ignorated);
        pt1.forEach((p) -> {
            pt.add(p);
        });
        pt2.forEach((p) -> {
